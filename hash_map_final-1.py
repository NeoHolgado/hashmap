    ...
    def resize_table(self, capacity):
        ...
        for bucket in self._buckets:
            ...
            while current is not None:
                # Calculate index for each key for the temporary hashMap
                # and put the key and value in the temporary hashMap
                index = self._hash_function(current.key) % capacity
                temp._buckets[index].add_front(current.key, current.value)
